...
public class MediaData implements MediaDataSourceI {
    ...
    byte[] artBytes, displayIconBytes, albumArtBytes;
    Bitmap art, displayIcon, albumArt;
    ...
    public MediaData(Bundle bundle) {
        ...
        // media的插图
        artBytes = bundle.getByteArray(MediaMetadataCompat.METADATA_KEY_ART);
        if (artBytes != null) {
        ...
        // 显示的小图标
        displayIconBytes = bundle.getByteArray(MediaMetadataCompat.METADATA_KEY_DISPLAY_ICON);
        if (displayIconBytes != null) {
            displayIcon = BitmapFactory.decodeByteArray(displayIconBytes, 0, displayIconBytes.length);
        }
        // 显示小图标的路径
        ...
        // 专辑封面
        albumArtBytes = bundle.getByteArray(MediaMetadataCompat.METADATA_KEY_ALBUM_ART);
    ...
    public String toString() {
        return "MediaData{" +
                ...
                ", artBytes=" + (artBytes == null || artBytes.length == 0) +
                ", displayIconBytes=" + (displayIconBytes == null || displayIconBytes.length == 0) +
                ", albumArtBytes=" + (albumArtBytes == null || albumArtBytes.length == 0) +
                ", art=" + art +
